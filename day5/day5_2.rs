...
impl Line {
    ...
    fn is_up_right_diagonol(&self) -> bool {
        return self.end.x > self.start.x && self.end.y < self.start.y;
    }

    fn is_down_left_diagonol(&self) -> bool {
        return self.end.x < self.start.x && self.end.y > self.start.y;
    }

    fn is_down_right_diagonol(&self) -> bool {
        return self.end.x > self.start.x && self.end.y > self.start.y;
    }

    fn is_up_left_diagonol(&self) -> bool {
        return self.end.x < self.start.x && self.end.y < self.start.y;
    }
    ...
    fn get_all_points(&self) -> Vec<Point> {
        ...
        } else if self.is_up_right_diagonol() || self.is_down_left_diagonol() {
            for (i, x) in (self.min_x()..=self.max_x()).enumerate() {
                all_points.push(Point { x: x, y: self.max_y() - (i as u64) } );
            }
        } else if self.is_up_left_diagonol() || self.is_down_right_diagonol() {
            for (i, x) in (self.min_x()..=self.max_x()).enumerate() {
                all_points.push(Point { x: x, y: self.min_y() + (i as u64) } );
            }
        }
